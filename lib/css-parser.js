...
class CssParser {
  ...
  nuked() {
    ...
    this._parsed.traverse((rule, index, parent) => {
      ...
      if (rule.type === 'multilineComment') {
        if (ENABLE_COMMENT_REGEX.test(rule.content)) {
          enabled = true
          parent.removeChild(index)
        } else if (DISABLE_COMMENT_REGEX.test(rule.content)) {
          enabled = false
          parent.removeChild(index)
        }
      } else if (enabled && rule.type === 'ruleset') {
